...
EXPECTED_REFS = frozenset({"v2.1.8"})
...
def validate_context(context: dict[str, Any]) -> None:
    ...
    if source.get("ref") not in EXPECTED_REFS:
        raise ValueError("release requires source ref v2.1.8")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate the immutable fresh-only v2.1.8 AIDP lab release contract.")
