...
EXPECTED_REFS = frozenset({"v2.1.21"})
...
def validate_context(context: dict[str, Any]) -> None:
    ...
    if source.get("ref") not in EXPECTED_REFS:
        raise ValueError("release requires source ref v2.1.21")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate the immutable fresh-only v2.1.21 AIDP starter kit release contract.")
